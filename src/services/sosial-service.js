...
class SosialService {
  ...
  async kakaoFinish(code) {
    ...
    if (access_token) {
      ...
      // 액세스 토큰이 존재한다면 kapi~~ url로 요청을보내는데, 이때 cors이슈로 백엔드처리하는것이 좋다.
      const apiUrl = "https://kapi.kakao.com/v2/user/me";
      // profile이란 변수에 get요청한 값을 담는다.
      const profile = await axios.get(apiUrl, {
        // axios 요청에 액세스토큰을 헤더에 담아서 보낸다.
        headers: { Authorization: `Bearer ${access_token}` },
      });
      // 원래 콘솔로 profile 을 찍어보며 값을 추출해야한다.
      // 콘솔을 넣으면 터미널이 지져분해져서 뺏다.
